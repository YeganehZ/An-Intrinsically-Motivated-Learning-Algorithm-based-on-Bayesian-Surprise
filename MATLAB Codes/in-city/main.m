...
xpred0=[16; 0;0; 27; 13; 0;0];          % initial state estimation mean vector  
Ppred0=diag([100,1,1,100, 100,1,1]);    % initial state estimation covariance matrix 
distance=27.8;   % initial distance between two vehicles
B=10^(8);        % Bandwidth 
...
for mc=1:MC    % for each Monte Carlo simulation

% Select the first action from library at random 
...
Planning_ExpSurprise=zeros(R_Loc,K); % Computing the Expectation of Bayesian surprise for the planning stage 


% This part loads the MATLAB file for the state vector x_k that we have 
% implemented randomly for constant jerk (in-city driving experience)   
...
x=x_MC(:,:,mc);
 ...
 for j=2:K
   ...
   [Rnew, Rnewindx, maxRindx,policy,Valutogo_update,Planning_ExpSurprise,reward_Bayesian]=Planning_Bayesian_jerk(j,n,m,Pest(:,:,j),H,Q,F,ExpSurprise(1,j),R_Localized,R_Localized_indx,Valuetogo,alpha,gamma,epsilon,policy,Planning_ExpSurprise,TERM1_ExpSurp(1,j),TERM2_ExpSurp(1,j));
