function [Rnew, Rnewindx, max_Rindx,policy,Valutogo_update,Planning_ExpSurprise,reward_Bayesian]=Planning_Bayesian_jerk(cycle,n,m,Pest,H,Q,F,ExpSurprise_previous,R_Localized,R_Localized_indx,Valuetogo,alpha,gamma,epsilon,policy,Planning_ExpSurprise, TERM1_ExpSurp_prev,TERM2_ExpSurp_prev)
...
if cycle==2
 for i=1:R_Loc
  ...
  TERM3_ExpSurp(i)=det(pinv(R_Localized(:,:,i)*pinv(Spred_new(:,:,i)))-eye(m));
...
% reward_Bayesian(i)= (ExpSurprise_previous-ExpSurprise(i))/(a+ExpSurprise(i));
 ...
 reward_Bayesian(i)= ((TERM1_ExpSurp_prev + (ExpSurprise_previous*TERM2_ExpSurp_prev))/(1+ExpSurprise_previous))  -((TERM1_ExpSurp(i) + (ExpSurprise(i)*TERM2_ExpSurp(i)))/(1+ExpSurprise(i)));
  ...
  load(['C:\Users\Yeganeh\Documents\Kalman Filter Examples\January 2021\Autonomous Car\Radar\Making_Rmatrix_constantjerk\Rmatrix' Filename],'R_Localized','R_Localized_indx');
